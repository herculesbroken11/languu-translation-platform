...
import { LANGUAGES, SUPPORTED_VIDEO_FORMATS, MAX_FILE_SIZE } from '@/utils/constants';
...
const VideoUploadPanel: React.FC = () => {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!fileExtension || !SUPPORTED_VIDEO_FORMATS.includes(fileExtension)) {
      setError(`Unsupported file format. Supported formats: ${SUPPORTED_VIDEO_FORMATS.join(', ')}`);
      return;
    }
  ...
  const handleTranscribe = async () => {
    ...
    try {
      // Convert video file to audio format for transcription
